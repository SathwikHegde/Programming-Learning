...
insert into Employee (employee_id, name, experience_years) values ('4', 'Doe', '2')a
...
SELECT 
    project_id, employee_id
FROM (
SELECT p.project_id, p.employee_id, e.experience_years, max(e.experience_years) OVER (PARTITION BY project_id ORDER BY e.experience_years DESC) mx
FROM Project p
